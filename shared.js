...
// Proxy planning (KV : planning, congés, workers)
const PLAN_PROXY = "https://planning-proxy.receptionroulier.workers.dev";
// Proxy mail/PDF → défini dans mail-pdf.js
  ...
  if (module === 'mail') {
    body.innerHTML = `
      ...
      <div style="display:flex;align-items:center;gap:10px;margin-bottom:20px;">
        <button class="modal-btn" style="font-size:0.7rem;padding:7px 14px;white-space:nowrap;" onclick="gmailAuthorize()">&#128275; Autoriser Gmail</button>
        <span style="font-size:0.65rem;color:var(--text3);">${state.config.gmailAuthorized ? '✅ Autorisé — token stocké sécurisément.' : '⚠️ Non autorisé — cliquez pour connecter.'}</span>
      </div>
